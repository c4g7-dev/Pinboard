 ...
 * Uses cfwidget API for CurseForge URLs (Cloudflare blocks direct scraping).
 */

/**
 * Extract OG-like metadata from CurseForge via cfwidget API.
 */
async function fetchCurseForge(url) {
  const result = { og_title: null, og_desc: null, og_image: null };
  try {
    // Extract path after curseforge.com: e.g. /minecraft/mc-mods/jei
    ...
    if (data.title) result.og_title = data.title.slice(0, 200);
    if (data.summary) result.og_desc = data.summary.slice(0, 500);
    if (data.thumbnail) {
      try {
        const imgUrl = new URL(data.thumbnail);
...
async function fetchOG(url) {
  ...
  try {
    ...
    // CurseForge: use API (Cloudflare blocks direct scraping)
    if (parsed.hostname === 'www.curseforge.com' || parsed.hostname === 'curseforge.com') {
      return fetchCurseForge(url);
    }
